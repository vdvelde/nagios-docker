...
# service definitions for nagios/nagios from passively received
# host and service checks.
#  
# This might be useful if your nagios/nagios system runs in a
# distributed environment with nagios/nagios instances sending
# passive checks to a central nagios/nagios server or if you
# have many hosts sending passive checks.
# 
# Normally, hosts and services based on passive checks must
# be configured both on the nagios/nagios instance receiving
...
# definitions in the nagios/nagios configuration. This way
...
# work together. You can run a Munin server and a nagios/nagios
# server in parallel. The munin server sends its results as
# passive checks to the nagios/nagios server. Then the
# corresponding service and host definitions are generated 
# and included in the nagios/nagios configuration. 
#
#
# HOW DOES IT WORK?
# -------------------------------------------------------------------
# The instance that sends passive checks must send them to
# the NSCA of the central nagios/nagios server. So be sure
# you have configured the NSCA correctly
#
# When a pssive check result is received by the nagios/nagios
# instance and the corresponding host and service respectively
# does not exist in the nagios/nagios configuration an error
# occurs and is written to the nagios/nagios log file.
#
# the radar script parses the nagios/nagios log file for these
# errors and uses them to generate automated host and service
# definitions in the nagios/nagios configuration.
#
# As long as nagios/nagios is not reloaded passive host and
# service checks keep causing errors in the nagios/nagios
...
# nagios/nagios configuration.
#
# As the nagios/nagios configuration may be very large the script
...
# nagios/nagios configuration. The names of the templates are set
...
# CONFIGURATION
# -------------------------------------------------------------------
# absolute path to the nagios/nagios log file
...
# nagios/nagios cfg files are kept
# $NAGIOS_CONFIG="/usr/local/nagios/etc/objects/";
#
# the nagios/nagios host template to use for generated
# host definitions
# $HOST_TEMPLATES="generic-radar-host";
#
# the nagios/nagios service template to use for
# generated service definitions
# $SERVICE_TEMPLATES="generic-radar-service";
#
# the nagios/nagios user
# $NAGIOS_USER="nagios";
#
# the nagios/nagios group
...
# absolute path to the nagios/nagios log file
my $NAGIOS_LOGFILE="/usr/local/nagios/var/nagios.log";

# the absolute path to the directory where generated cfg-files
# are to be stored
my $CFG_DIRECTORY="/usr/local/nagios/etc/objects/radar";

# the absolute path to the directory where the
# nagios/nagios cfg files are kept
my $NAGIOS_CONFIG="/usr/local/nagios/etc/objects/";
...
# directory to put the log file
my $LOGFILE_DIRECTORY="/usr/local/radar/log/";
...
my $CFG_FILE_PREFIX="radar";

# a regular expression to find a new host from the nagios/nagios
# logging output
#my $HOST_PATTERN=qr/Passive\s+check\s+result.+received\s+for\s+host\s+'(.+)'.+not.+found/i;
my $HOST_PATTERN=qr/Got\s+host\s+checkresult\s+for\s+'(.+)',\s+but\s+no\s+such\s+host\s+can\s+be\s+found/i;

# a regular expression to find a new service from the
# nagios/nagios logging output
#my $SERVICE_PATTERN=qr/Passive\s+check\s+result.+received\s+for\s+service\s+'(.+)'\s+on\s+host\s+'(.+)'.+not.+found/i;
my $SERVICE_PATTERN=qr/Got\s+check\s+result\s+for\s+service\s+'(.+)'\s+on\s+host\s+'(.+)'.\s+Unable\s+to\s+find\s+service/i;

# the time interval in seconds to use for searching the nagios/nagios
...
# read the nagios log file and store it to an array
sub readLogFile {
...
# helper function to get nagios user id
sub getIcingaUserId {

   (my $name, my $pass, my$uid, my $gid, my $quo, my $com, my $gc, my $dir, my $sh,my $exp) = getpwnam($NAGIOS_USER);
   return $uid;
}


# helper function to get nagios group id
sub getIcingaGroupId {

   (my $name, my $pass, my $gid, my $members)=getgrnam($NAGIOS_GROUP);
   return $gid;
}
